...
public class HashToPolyTest {
        ...
        public void testHashPointsToPolyLength() throws NoSuchAlgorithmException {
                ...
                for (int i = 1; i <= 10; i++) { // run 10 test with random values of t and n, but always with the
                                                // property n - t
                                                // - 2 <= 0.
                        int t;
                        int n;
                        ...
                        for (int j = 1; j <= 10; j++) {
                                if (i == j) {
                                        ...
                                        DistributionInput distInput = DistributionInputGenerator
                                                        .generateDistributionInput(ctx);
                                        ...
                                        for (int k = 0; k < n; k++) {
                                                // For instance, if you want to use each participant’s public key as
                                                // their
                                                // commitment key:
                                        ...
                                        assertEquals("Polynomial coefficients array length", numPolyCoeffs,
                                                        polyCoeffs.length);
                ...
                for (int i = 1; i <= 10; i++) { // run 10 test with random values of t and n, but always with the
                        ...
                        for (int j = 1; j <= 10; j++) {
                                if (i == j) {
                                        ...
                                        BigInteger modulus = ctx.getOrder();
                                        int numPolyCoeffs = 5;
                                        BigInteger[] polyCoeffs1 = HashingTools.hashPointsToPoly(dealerPub, comKeys,
                                                        encryptedShares,
                                                        numPolyCoeffs,
                                                        modulus);
                                        BigInteger[] polyCoeffs2 = HashingTools.hashPointsToPoly(dealerPub, comKeys,
                                                        encryptedShares,
                                                        numPolyCoeffs,
                                                        modulus);
                                        assertArrayEquals("Hash-to-poly must be deterministic", polyCoeffs1,
                                                        polyCoeffs2);
                                        System.out.println("Test no " + i + " out of 10 is completed succesfully");
                                }
                        }
        ...
        public void testHashPointsToPolyAndEvaluation() throws NoSuchAlgorithmException {
                ...
                BigInteger[] evaluations = EvaluationTools.evalAll(polyCoeffs1, ctx.getAlphas(), modulus);
